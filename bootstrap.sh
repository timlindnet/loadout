...
#
# Usage (stdin + args):
#   curl -fsSL https://loadout.timlind.net | bash -s -- --dev --gaming
#   wget -qO-  https://loadout.timlind.net | bash -s -- --dev --gaming
...
case "$fetcher" in
  curl) curl -fsSL "$url" | bash -s -- "$@" ;;
  wget) wget -qO- "$url" | bash -s -- "$@" ;;
  *) die "Unknown fetcher: $fetcher" ;;
esac
EOF
  chmod +x "$tmp_helper"
...
install_loadout_helper "$LOADOUT_URL" "$fetcher"
